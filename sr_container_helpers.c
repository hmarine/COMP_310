...
int setup_child_capabilities()
{
    ...
    int drop_caps[] = {
        ...
        CAP_DAC_READ_SEARCH, CAP_FSETID, CAP_IPC_LOCK, CAP_MAC_ADMIN, CAP_MAC_OVERRIDE, 
        CAP_MKNOD, CAP_SETFCAP, CAP_SYSLOG, CAP_SYS_ADMIN, CAP_SYS_BOOT, CAP_SYS_MODULE, 
        CAP_SYS_NICE, CAP_SYS_RAWIO, CAP_SYS_RESOURCE, CAP_SYS_TIME, CAP_WAKE_ALARM
    };
    ...
    int clear_inh_set = cap_set_flags(caps, CAP_INHERITABLE, num_caps, drop_caps, CAP_CLEAR);
    if (clear_inh_set) {
        perror("cap_set_flag");
        cap_free(caps);
        return EXIT_FAILURE;
...
int setup_syscall_filters() {
    ...
    if (filter_set_status) {
        ...
        fprintf(stderr, "seccomp could not add KILL rule for 'mbin': %m\n");
        return EXIT_FAILURE;
    }
    ...
    filter_set_status = seccomp_rule_add(
        ...
        SCMP_SYS(chmod),
        1,
        SCMP_A1(SCMP_CMP_MASKED_EQ, S_ISUID, S_ISGID)
    );
    if (filter_set_status) {
        if (seccomp_ctx) {
            seccomp_release(seccomp_ctx);
    ...
    filter_set_status = seccomp_attr_set(
        seccomp_ctx, 
        SCMP_FLTATR_CTL_NNP, 
        0
    );
    if (filter_set_status) {
        ...
        fprintf(stderr, "seccomp could not set attributes 'SCMP_FLTATR_CTL_NNP': %m\n");
        return EXIT_FAILURE;
    }
