...
struct cgroups_control *cgroups[5] = {
	& (struct cgroups_control) {
		.control = CGRP_BLKIO_CONTROL,                      //name of the cgroup
		.settings = (struct cgroup_setting *[]) {           //setting for the cgroup
			& (struct cgroup_setting) {                     //append first setting (that is a struct of type cgroup_setting) to cgroup_setting array
				.name = "blkio.weight",
				.value = "64"
			},
			&self_to_task,      //second setting              // must be added to all the new controls added
			NULL                //third setting               // NULL at the end of the array
		}
	},
